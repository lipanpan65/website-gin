...
func NewTopicHandler(topicService services.TopicServiceInterface) *TopicHandler {
	return &TopicHandler{
...
func (h *TopicHandler) CreateTopic(ctx *gin.Context) {
	...
	if err := ctx.ShouldBind(&topicDTO); err != nil {
		utils.ResultError(ctx, errors.CantChangeAdmin)
		return
	}
	topicVo, err := h.topicService.CreateTopic(&topicDTO)
	if err != nil {
...
func (h *TopicHandler) QueryTopicByID(ctx *gin.Context) {
	var id uint
	...
	topicVo, err := h.topicService.QueryTopicByID(id)
	if err != nil {
		utils.ResultError(ctx, err.Error())
		return
	}
...
func (h *TopicHandler) QueryTopics(ctx *gin.Context) {
	// 获取分页参数
	...
	if err != nil || page < 1 {
		utils.ResultError(ctx, errors.ParameterError)
		return
	}
	...
	// 调用服务层方法进行分页查询
	topics, total, err := h.topicService.QueryTopics(conditions, page, pageSize)
	if err != nil {
		utils.ResultError(ctx, err)
		return
	}
